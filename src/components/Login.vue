...
<script>
import rotas from './../RotasServico'
...
export default {
    ...
    methods: {
        async submit () {
            ...
            return this.$http
                .post(rotas.get('/login'), formData, { emulateJson : true })
                .then(
                    response => {
                    ...
                    error => {
                        console.log(error)
                        if(error.status == 401) {
                            bus.$emit('mensagem-alerta', 'E-mail e ou senha inválidos')
                        } else {
                            bus.$emit('mensagem-alerta', 'Erro ao tentar efetuar login')
                        }
                        this.carregando = false
                    }
                )
